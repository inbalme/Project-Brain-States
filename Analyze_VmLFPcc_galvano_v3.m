...
subplot(2*length(trace),1,2*tr_ind)
    hold on
        ...
        if trace_type==2;
            fn_plot_sensory_stim(dt, stim2{1},whisker_stim_color)
            xlm=get(gca,'xlim'); xlm(2)=end_sample(1).*dt*1000; set(gca,'xlim',xlm); %showing only the part of trace that was used for CC analysis
        end
