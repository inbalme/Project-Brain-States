...
%evoked
evoked_trace_Off_f44_zoom = open('f44_traces_x2_v2.fig');    
b1=findall(gcf,'type','line');
b2=findall(gcf,'type','text');
delete(b1(2,1)); 
delete(b2(2,1));
evoked_trace_Off_f44_zoom_ax = get(gcf, 'children');

evoked_trace_On_f44_zoom = open('f44_traces_x3_v2.fig');  
b1=findall(gcf,'type','line');
b2=findall(gcf,'type','text');
delete(b1(2,1)); 
delete(b2(2,1));
evoked_trace_On_f44_zoom_ax = get(gcf, 'children');

evoked_mean_f44_zoom = open('f44_mean_x2+3_v2.fig');    
evoked_mean_f44_zoom_ax = get(gcf, 'children');

evoked_std_f44_zoom = open('f44_std_x2+3_mean-subt_v2.fig');    
b1=findall(gcf,'type','line');
b2=findall(gcf,'type','text');
delete(b1(2,1)); 
delete(b2(2,1));
evoked_std_f44_zoom_ax = get(gcf, 'children');
...
v_dist=0.04;
evoked_trace_Off_f44_zoom_pos(1,:) = [0.04 , 0.72 ,  0.44 ,  0.20];
evoked_trace_On_f44_zoom_pos(1,:) = [evoked_trace_Off_f44_zoom_pos(1,1), 0.5, evoked_trace_Off_f44_zoom_pos(1,3),evoked_trace_Off_f44_zoom_pos(1,4)]; 
        evoked_trace_On_f44_zoom_pos(1,2)=evoked_trace_Off_f44_zoom_pos(1,2)-evoked_trace_On_f44_zoom_pos(1,4)-v_dist;
evoked_mean_f44_zoom_pos(1,:) = [evoked_trace_Off_f44_zoom_pos(1,1), evoked_trace_On_f44_zoom_pos(1,2)-0.10-v_dist, evoked_trace_Off_f44_zoom_pos(1,3), 0.10];
evoked_std_f44_zoom_pos(1,:) = [evoked_trace_Off_f44_zoom_pos(1,1), evoked_mean_f44_zoom_pos(1,2)-evoked_mean_f44_zoom_pos(1,4)-v_dist, evoked_trace_Off_f44_zoom_pos(1,3), evoked_mean_f44_zoom_pos(1,4)];

v_dist2=0.05;
Peak_amp_pos(1,:)=[0.05, 0.5, 0.12, 0.10];  Peak_amp_pos(1,2)=evoked_std_f44_zoom_pos(1,2)-Peak_amp_pos(1,4)-v_dist2;
% VmM_pos(1,:)=[0.05, 0.5, 0.08, 0.10]; VmM_pos(1,2)=evoked_std_f44_zoom_pos(1,2)-VmM_pos(1,4)-v_dist2;
% VmSTD_pos(1,:)= VmM_pos; VmSTD_pos(1,1)=VmM_pos(1,1)+VmM_pos(1,3)+0.05;
SNR_pos(1,:)= [0.5, Peak_amp_pos(1,2),0.06, 0.1]; SNR_pos(1,1)=Peak_amp_pos(1,1)+Peak_amp_pos(1,3)+0.06; 
Amplitude_Signal_pos(1,:)=SNR_pos; Amplitude_Signal_pos(1,1)=SNR_pos(1,1)+SNR_pos(1,3)+0.04;
Amplitude_Noise_pos(1,:)=Amplitude_Signal_pos; Amplitude_Noise_pos(1,1)=Amplitude_Signal_pos(1,1)+Amplitude_Signal_pos(1,3)+0.04;

%ChAT
h_dist1=evoked_trace_Off_f44_zoom_pos(1,1)+evoked_trace_Off_f44_zoom_pos(1,3)+0.1;

evoked_trace_Off_f80_zoom_pos(1,:) = [h_dist1, evoked_trace_Off_f44_zoom_pos(1,2) ,  evoked_trace_Off_f44_zoom_pos(1,3) ,  evoked_trace_Off_f44_zoom_pos(1,4)];
evoked_trace_On_f80_zoom_pos(1,:) = [h_dist1, evoked_trace_Off_f80_zoom_pos(1,2)-evoked_trace_Off_f80_zoom_pos(1,4)-0.02 ,  evoked_trace_Off_f80_zoom_pos(1,3) ,  evoked_trace_Off_f80_zoom_pos(1,4)];
evoked_mean_f80_zoom_pos(1,:) = [h_dist1, evoked_trace_On_f80_zoom_pos(1,2)-0.10-0.03 ,   evoked_trace_Off_f80_zoom_pos(1,3) ,   evoked_mean_f44_zoom_pos(1,4)];
evoked_std_f80_zoom_pos(1,:) = [h_dist1, evoked_mean_f80_zoom_pos(1,2)-evoked_mean_f80_zoom_pos(1,4)-0.03 ,  evoked_trace_Off_f80_zoom_pos(1,3) ,  evoked_std_f44_zoom_pos(1,4)];

v_dist2=0.05;
Peak_amp_ChAT_pos(1,:)=[h_dist1, 0.5, 0.12, 0.10];  Peak_amp_ChAT_pos(1,2)=evoked_std_f44_zoom_pos(1,2)-Peak_amp_ChAT_pos(1,4)-v_dist2;
% VmM_pos(1,:)=[0.05, 0.5, 0.08, 0.10]; VmM_pos(1,2)=evoked_std_f44_zoom_pos(1,2)-VmM_pos(1,4)-v_dist2;
% VmSTD_pos(1,:)= VmM_pos; VmSTD_pos(1,1)=VmM_pos(1,1)+VmM_pos(1,3)+0.05;
SNR_ChAT_pos(1,:)= [0.5, Peak_amp_ChAT_pos(1,2),0.06, 0.1]; SNR_ChAT_pos(1,1)=Peak_amp_ChAT_pos(1,1)+Peak_amp_ChAT_pos(1,3)+0.06; 
Amplitude_Signal_ChAT_pos(1,:)=SNR_ChAT_pos; Amplitude_Signal_ChAT_pos(1,1)=SNR_ChAT_pos(1,1)+SNR_ChAT_pos(1,3)+0.04;
Amplitude_Noise_ChAT_pos(1,:)=Amplitude_Signal_ChAT_pos; Amplitude_Noise_ChAT_pos(1,1)=Amplitude_Signal_ChAT_pos(1,1)+Amplitude_Signal_ChAT_pos(1,3)+0.04;

%position of top of each panel
evoked_trace_Off_f44_zoom_pos_top = evoked_trace_Off_f44_zoom_pos(1,2)+evoked_trace_Off_f44_zoom_pos(1,4);
evoked_trace_On_f44_zoom_pos_top = evoked_trace_On_f44_zoom_pos(1,2)+evoked_trace_On_f44_zoom_pos(1,4);
evoked_mean_f44_zoom_pos_top = evoked_mean_f44_zoom_pos(1,2)+evoked_mean_f44_zoom_pos(1,4);
evoked_std_f44_zoom_pos_top = evoked_std_f44_zoom_pos(1,2)+evoked_std_f44_zoom_pos(1,4);
...
evoked_trace_Off_f44_zoom_ax_copy = copyobj(evoked_trace_Off_f44_zoom_ax,F); % copy axes to new fig
set(evoked_trace_Off_f44_zoom_ax_copy,'position',evoked_trace_Off_f44_zoom_pos(1,:))

evoked_trace_On_f44_zoom_ax_copy = copyobj(evoked_trace_On_f44_zoom_ax,F); % copy axes to new fig
set(evoked_trace_On_f44_zoom_ax_copy,'position',evoked_trace_On_f44_zoom_pos(1,:))

evoked_mean_f44_zoom_ax_copy = copyobj(evoked_mean_f44_zoom_ax,F); % copy axes to new fig
set(evoked_mean_f44_zoom_ax_copy,'position',evoked_mean_f44_zoom_pos(1,:))

evoked_std_f44_zoom_ax_copy = copyobj(evoked_std_f44_zoom_ax,F); % copy axes to new fig
set(evoked_std_f44_zoom_ax_copy,'position',evoked_std_f44_zoom_pos(1,:))
...
%position legend:
set(evoked_mean_f44_zoom_ax_copy(1),'position',[0.4 evoked_trace_Off_f44_zoom_pos_top+0.03 0.08 0.005])
evoked_mean_f44_zoom_ax_copy(1).FontSize=10;
% set(evoked_mean_f44_zoom_ax_copy(1),'position',[0.9 spont_std_f44_pos_top+0.01 0.08 0.05])
...
set(F, 'currentaxes', Peak_amp_ChAT_ax_copy); tl=title(''); yl=ylabel('Amp. [mV]');
...
%position legend:
set(evoked_mean_f80_zoom_ax_copy(1),'position',[0.85 evoked_trace_Off_f80_zoom_pos_top+0.03 0.08 0.005])
evoked_mean_f80_zoom_ax_copy(1).FontSize=10;


%   annotation:
% annotation('textbox', [evoked_trace_Off_f44_zoom_pos(1,1) evoked_trace_Off_f44_zoom_pos_top 0 0]+[0.03 0.01 0.5 0.05],...
%      'FitHeightToText', 'on', 'edgecolor', 'none','string', 'Sensory-evoked Responses', 'FontName','arial', 'fontsize', 14, 'fontweight', 'bold')
% annotation('textbox', [evoked_trace_Off_f44_zoom_pos(1,1), evoked_trace_Off_f44_zoom_pos_top 0 0]+[0.06 0.01 0.5 0.05],...
%      'FitHeightToText', 'on', 'edgecolor', 'none','string', 'Spontaneous Activity', 'FontName','arial', 'fontsize', 14, 'fontweight', 'bold')
 ...
 annotation('textbox', [evoked_trace_Off_f44_zoom_pos(1,1) evoked_trace_Off_f44_zoom_pos_top 0 0]+a_pos4,...
     'FitHeightToText', 'on', 'edgecolor', 'none','string', 'Vm Traces', 'FontName','arial', 'fontsize', 12) %'fontweight', 'bold'
annotation('textbox', [evoked_mean_f44_zoom_pos(1,1) evoked_mean_f44_zoom_pos_top 0 0]+a_pos4,...
    'FitHeightToText', 'on', 'edgecolor', 'none','string', 'Mean', 'FontName','arial', 'fontsize', 12)
annotation('textbox', [evoked_std_f44_zoom_pos(1,1), evoked_std_f44_zoom_pos_top 0 0]+a_pos4,...
...
annotation('textbox', [spont_trace_f44_pos(1,1) spont_trace_f44_pos_top 0 0]+a_pos1,...
     'FitHeightToText', 'on', 'edgecolor', 'none','string', 'A', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')
annotation('textbox', [spont_mean_f44_pos(1,1) spont_mean_f44_pos_top 0 0]+a_pos1,...
    'FitHeightToText', 'on', 'edgecolor', 'none','string', 'B', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')
annotation('textbox', [spont_std_f44_pos(1,1) spont_std_f44_pos_top 0 0]+a_pos1,...
    'FitHeightToText', 'on', 'edgecolor', 'none','string', 'C', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')

annotation('textbox', [evoked_trace_Off_f44_zoom_pos(1,1) evoked_trace_Off_f44_zoom_pos_top 0 0]+a_pos2,...
    'FitHeightToText', 'on', 'edgecolor', 'none','string', 'D', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')
annotation('textbox', [evoked_trace_On_f44_zoom_pos(1,1) evoked_trace_On_f44_zoom_pos_top 0 0]+a_pos2,...
    'FitHeightToText', 'on', 'edgecolor', 'none','string', 'E', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')
annotation('textbox', [evoked_mean_f44_zoom_pos(1,1) evoked_mean_f44_zoom_pos_top 0 0]+a_pos2,...
    'FitHeightToText', 'on', 'edgecolor', 'none','string', 'F', 'FontName','arial', 'fontsize', 12, 'fontweight', 'bold')
annotation('textbox', [evoked_std_f44_zoom_pos(1,1) evoked_std_f44_zoom_pos_top 0 0]+a_pos2,...
