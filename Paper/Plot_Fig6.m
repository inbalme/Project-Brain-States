...
v_dist=0.12;
VmM_pos(1,:)=[0.08, 0.68, 0.1, 0.13]; 
VmSTD_pos(1,:)= VmM_pos; VmSTD_pos(1,1)=VmM_pos(1,1)+VmM_pos(1,3)+0.09;
Peak_amp_pos(1,:)=[VmM_pos(1,1), 0.5, 0.38, 0.2 ]; Peak_amp_pos(1,2)=VmM_pos(1,2)-Peak_amp_pos(1,4)-v_dist;
% Peak_amp_pos(1,:)=[0.5,VmM_pos(1,2)+0.03, 0.15, VmM_pos(1,4)-0.03];  Peak_amp_pos(1,1)=VmSTD_pos(1,1)+VmSTD_pos(1,3)+0.08;

SNR_pos(1,:)= [VmM_pos(1,1), 0.5,0.09, 0.13]; SNR_pos(1,2)=Peak_amp_pos(1,2)-SNR_pos(1,4)-v_dist-0.05;
...
SNR_ChAT_pos(1,:)= [VmM_ChAT_pos(1,1), SNR_pos(1,2),0.09, 0.13]; 
Amplitude_Signal_ChAT_pos(1,:)=SNR_ChAT_pos; Amplitude_Signal_ChAT_pos(1,1)=SNR_ChAT_pos(1,1)+SNR_ChAT_pos(1,3)+0.06;
Amplitude_Noise_ChAT_pos(1,:)=Amplitude_Signal_ChAT_pos; Amplitude_Noise_ChAT_pos(1,1)=Amplitude_Signal_ChAT_pos(1,1)+Amplitude_Signal_ChAT_pos(1,3)+0.06;
...
set(F, 'currentaxes', VmM_ax_copy(1));  tl=title(''); yl=ylabel('mV'); xl=xlabel(''); %tl=title('Mean Vm','fontsize',11,'fontweight','normal');
...
set(F, 'currentaxes', Peak_amp_ax_copy); tl=title(''); yl=ylabel('Amp. Norm.'); xl=xlabel('Stim. #');
...
set(F, 'currentaxes', SNR_ax_copy);  tl=title('');  yl=ylabel(''); %tl=title('SNR','fontsize',11,'fontweight','normal');
SNR_ax_copy.FontSize=ax_fontsize;
 set(gca,'tickdir','out','ylim',[0 4],'ytick',[0,2,4])
ticklen=fn_get_abs_ticklength(gca, abslen);
...
set(F, 'currentaxes', Amplitude_Signal_ax_copy); tl=title('');  yl=ylabel('mV'); %tl=title('Signal Amp.','fontsize',11,'fontweight','normal');
...
set(F, 'currentaxes', Amplitude_Noise_ax_copy); tl=title('');  yl=ylabel('mV');  %tl=title('Noise Amp.','fontsize',11,'fontweight','normal');
...
set(F, 'currentaxes', VmM_ChAT_ax_copy(1));  tl=title(''); yl=ylabel('mV'); xl=xlabel(''); %tl=title('Mean Vm','fontsize',11,'fontweight','normal');
...
set(F, 'currentaxes', VmSTD_ChAT_ax_copy);  tl=title('');yl=ylabel('mV'); xl=xlabel('');%tl=title('STD Vm','fontsize',11,'fontweight','normal'); 
...
set(F, 'currentaxes', Peak_amp_ChAT_ax_copy); tl=title(''); yl=ylabel('Amp. Norm.'); xl=xlabel('Stim. #');
...
set(F, 'currentaxes', SNR_ChAT_ax_copy);  tl=title('');  yl=ylabel(''); %tl=title('SNR','fontsize',11,'fontweight','normal');
...
set(F, 'currentaxes', Amplitude_Signal_ChAT_ax_copy); tl=title('');  yl=ylabel('mV'); %tl=title('Signal Amp.','fontsize',11,'fontweight','normal');
...
set(F, 'currentaxes', Amplitude_Noise_ChAT_ax_copy); tl=title('');  yl=ylabel('mV');  %tl=title('Noise Amp.','fontsize',11,'fontweight','normal');
...
annotation('textbox', [Amplitude_Signal_pos(1,1)-0.02 Amplitude_Signal_pos_top+0.005 0.16 0.04],...
...
annotation('textbox', [Amplitude_Noise_ChAT_pos(1,1)-0.065 Amplitude_Noise_ChAT_pos_top+0.07 0 0],...
  ...
  if save_flag==1
    filename='Fig6_f46_f80';
    saveas(F,filename,'fig'); 
    print(F,filename,'-dpng','-r600','-opengl') 
    print -depsc2 Fig6_f46_f80
