...
for header = 1:3;
    ...
    sf_Ext1 = Param.header(header).stim.sf_Ext1; %[1/sec]
    dt_Ext1 = 1/sf_Ext1;
    time_axis_Ext1 = [dt_Ext1:dt_Ext1:size(data.header(header).x_value(2).Ext1,1)*dt_Ext1];
    
    
    LFP_mean_puff_only = data.header(header).x_value(1).Ext1_mean;
    LFP_mean_puff_light = data.header(header).x_value(2).Ext1_mean;

    laser_vec(data.header(header).x_value(2).laser(:,1) > 2)=1.1*max(LFP_mean_puff_only);
 
figure(header)
        set(gcf,'color','w')
        subplot(2,1,1)
            hold on 
            plot(time_axis_Ext1, LFP_mean_puff_only, '-b', 'linewidth', 2)
%             plot(LFP_mean_puff_only - std(LFP_mean_puff_only),'k');
            plot(time_axis_Ext1, LFP_mean_puff_light, '-r', 'linewidth', 1)
            plot(time_axis_laser(laser_vec~=0), laser_vec(laser_vec~=0), '.k')
             set( gca, 'xtick', [], 'ticklength', [0.010 0.010],'fontname', 'helvetica', 'fontweight', 'bold', 'box', 'off' );
            hold off
