...
function [F,obj] = fn_Plot_Trace_std(raw_data, data_std, trace_to_plot,dt_data, dt_stim, stim_X);
...
max_data = max(max(Poly_Y(:,trace_to_plot)));
min_data = min(min(Poly_Y(:,trace_to_plot)));
...
for i = 1:length(trace_to_plot);
    ...
    obj(i)=plot(X_axis,raw_data(:,trace_to_plot(i)), 'LineWidth',0.5,'color', [0 0 0]); %color_table(i,:));
end
if nargin == 6
