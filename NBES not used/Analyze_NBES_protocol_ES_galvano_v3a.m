%% Analyze NBES protocol ES+galvano v3.1
...
int_time=50; %[ms]
    ...
    clear data_mean data_std data_mean_smooth mean_peak_val_all mean_peak_loc_all mean_peak_val mean_peak_loc baseline_interval
...
for x_value = 2:3;
    ...
    data_mean_smooth=sgolayfilt(data_mean, 1, 29);
            for stim_num=1:size(stim2_X{x_value},2);
                min_res_del=4; %[ms]
                response_begin=stim2_X{x_value}(1,stim_num)+(min_res_del./1000).*sf_galvano; %time onset of whisker stim.+ minimum response latency.
                response_end=response_begin+int_time.*sf_galvano./1000;  
                baseline_time=3; %[ms]
                baseline_interval(:,1)=[stim2_X{x_value}(1,stim_num)-(baseline_time./1000).*sf_galvano:stim2_X{x_value}(1,stim_num)-1];
                data_std = std(data_no_spikes{channel}(baseline_interval,:,x_value),0,1);
                minpeak=mean(data_mean_smooth(baseline_interval))+2.*mean(data_std);
                ...
                mean_peak_val{x_value}(stim_num,:)=mean_peak_val_all{x_value,stim_num}(1);
                mean_peak_loc{x_value}(stim_num,:)=mean_peak_loc_all{x_value,stim_num}(1);
                mean_peak_loc{x_value}(stim_num,:)=mean_peak_loc{x_value}(stim_num,:)+response_begin;
            end             
   ...
   % verification of detected peaks in the raw data:
            for trace = 1:size(data_no_spikes{1},2)         
                                 h1=plot([1:size(raw_data{channel},1)].*dt, raw_data{channel}(:,trace,x_value),'b');
                                 h2=scatter(mean_peak_loc{x_value}.*dt,raw_data{channel}(mean_peak_loc{x_value},trace,x_value),'r', 'fill');
                                 pause
                                 delete(h1)
                                 delete(h2)
                                 figure(Fig6)
                        ...
                        hold off
                        close (gcf)
peaks.mean(1,x_value) = mean_peak_val{1,x_value};
peaks.mean_loc(1,x_value) = mean_peak_loc{1,x_value};
peaks.single_trace(:,:,x_value)=raw_data{channel}(mean_peak_loc{x_value},:,x_value);
peaks.mean_latency(:,x_value)=(mean_peak_loc{1,x_value}-stim2_X{x_value}(1,:)').*dt;
