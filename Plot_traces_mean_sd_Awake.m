...
no_DC_flag=1;
lWidth=1;
plot_trace=[12,9,8]; %f129 [12,9,8]; f121 [7,35,36]; f130 [31,35,38]
choose_traces=[1:37]; %[f121; 1:37]; %if choose_traces is empty the script takes all traces for analysis. If it is not empty the script takes the specified traces.
...
lengthh_vert=[2,5,20; 2,5,20];%[2,5,10; 2,5,10]; %[0.1,0.1,0.5];%f46:[2,5,10; 2,5,10]; f80, f121:[2,5,20; 2,5,20], f127:[1,2,10; 1,2,10]; f130 [2,2,20; 2,2,10] %lengthh_vert(1) is for STD, lengthh_vert(2) is for mean and (3) is for traces. row 1 spont. row 2 evoked
trace_ind =[3,5,7,8,10]; %[2,3,4,5,6];%[2,3,4,5,6]; %if trace_ind is empty, the default would be to take all traces
DC_factor =20; %12; %sets the spacing between plotted traces. set DC_factor=1 for LFP [f44 13; f46 16] [f80 9; f74 12; f121 20; f127 20; f126 30; 128 8; 130 60] 
...
switch exp_type
    ...
    case 2
        files_to_analyze =121; %129; %[74,76,77,80,82,84,87,90,92,112,114,115]; [94,96,98,101] [118,120,121,122,126,127,128,129]
...
%% pairs of Vm and whisking signal

plot_data_Vm=current_data_filt(:,:,x_value); %current_data_filt %current_data_no_DC 
plot_data_Vm_mean = nanmean(plot_data_Vm,2);  
plot_data_Vm_std =  nanstd(plot_data_Vm(:,:,x_value),0,2);  %current_data_no_DC
plot_data_whisk=data.x_value.whisk; %current_data_filt %current_data_no_DC 
plot_data_whisk_mean = nanmean(plot_data_whisk,2);  
plot_data_whisk_std =  nanstd(plot_data_whisk(:,:,x_value),0,2);  %current_data_no_DC

%% cross correlation of whisking and laser
 for trace = 1:size(plot_data_Vm,2);
        [cc{fileind}(:,trace),lags{fileind}(:,1)] = xcorr(plot_data_whisk(:,trace)-mean(plot_data_whisk(:,trace)),data.x_value.laser(:,trace)./3,'coeff') ; 
...
cc_mean{fileind}(:,1) = mean(cc{fileind}(:,:),2);

figure
plot(lags{fileind}(:,1),cc_mean{fileind}(:,1))
%%
wh=figure;
subplot(2,1,1)
plot([segment1*dt, segment2*dt],data.x_value.laser(:,1)./3,'color',color_table(2,:))
 set(gca, 'visible', 'off') ;
subplot(2,1,2)
plot([segment1*dt, segment2*dt],plot_data_whisk_mean,'k');
...
%  horiz_vert=0;        lengthh=lengthh_vert(1,1);     textit=[num2str(lengthh), ' '];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=0.03; perc2=[];
%         [b3,b4] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);    
    hold off
    set(gca, 'visible', 'off') ;
 
%% plotting Vm and whisk signal, 3 traces each.
...
ymaxwhisk=max([plot_data_whisk(:,plot_trace),plot_data_whisk(:,plot_trace)]);
yminwhisk=min([plot_data_whisk(:,plot_trace),plot_data_whisk(:,plot_trace)]);
ydiffwhisk=ymaxwhisk-yminwhisk;
yrangewhisk=ceil(max(ydiffwhisk));
...
%         p1=plot(interval_plot(:,1).*dt*1000,plot_data_Vm{1}(:,plot_trace(tr_ind)), 'color',color_table(1,:),'LineWidth',1.5);
        htrace1=plot(segment1*dt,plot_data_Vm(segment1,plot_trace(tr_ind)), 'LineWidth',1,'color', color_table(1,:));
        htrace12=plot(segment2*dt,plot_data_Vm(segment2,plot_trace(tr_ind)), 'LineWidth',1,'color', color_table(2,:));
%        htrace1=plot([segment1*dt, segment2*dt],plot_data_Vm(segment1(1):segment2(end),plot_trace(tr_ind)), 'LineWidth',1,'color', color_table(1,:));
%        rec1=rectangle('Position',[segment1(1,1)*dt,plot_data_Vm(segment1(1,1)),0.1,0.1]);
%         rec2=rectangle('Position',[segment1(1,1)*dt,plot_data_Vm(segment1(1,1)),0.1,0.1]);
...
uistack(htrace1,'top'); uistack(htrace12,'top');
...
horiz_vert=1;        lengthh=1;     textit=[num2str(lengthh), ' s'];     c=[0,0,0];  fonsizes=scalebar_fontsize; perc1=0.03; perc2=[];
        [b1,b2] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
horiz_vert=0;        lengthh=lengthh_vert(1,3);     textit=[num2str(lengthh), ' mV'];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=0.03; perc2=[];
        [b3,b4] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
...
%     uistack(htrace1,'top');
    ...
    subplot(2*length(plot_trace),1,2*tr_ind) 
 	hold on
        htrace2=plot([segment1*dt, segment2*dt],plot_data_whisk(segment1(1):segment2(end),plot_trace(tr_ind)), 'LineWidth',1,'color', color_table(3,:));
        
        axis tight            
     ylim_data=[get(gca,'ylim')]';
     ylim_data(1)=ylim_data(2)-yrangewhisk;   
     xlim_data=[get(gca,'xlim')]';
     ylim_data(2)=ylim_data(2)+(0.1).*yrangewhisk; 
     set(gca,'ylim',ylim_data, 'xlim',xlim_data)  
              
     %plotting scale bar
             horiz_vert=1;        lengthh=1;     textit=[num2str(lengthh), ' s'];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=[]; perc2=[];
                [b1,b2] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
             horiz_vert=0;        lengthh=50;     textit=[num2str(lengthh), ' a.u.'];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=[]; perc2=[];
                [b3,b4] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
 if tr_ind~=2;
             delete(b1); delete(b2); delete(b3); delete(b4);
         else
            ...
            %delete(b3); delete(b4); delete(b1); delete(b2);
...
%         set(line1(1),'xdata',linexdata-0.3);
    title('Vm-whisk single trace','FontSize', 16); 
    ylabel('Potential (mV)' ,'FontSize', 14); xlabel('Time (s)','FontSize', 14)
l=legend([htrace1,htrace12,htrace2],{'Vm light off','Vm light on','Whisk'}, 'box', 'off','position',[0.8,0.85,0.2,0.1]); 
    l.FontSize=11;

if save_flag==1;    
            cd(path_output1)
         filename= ['f' num2str(files_to_analyze(fileind)) '_VmWhisk_traces_x1'];
        saveas(pair1,filename,'fig') 
        print(pair1,filename,'-dpng','-r600','-opengl') 
end
%% plotting Vm, whisk signal and pupil size, 3 traces each.

% cd 'D:\Inbal M.Sc\Data PhD\ChAT Data\Extracted Data 2016\2017-10-16-001-H4-7-vid';
% %get the pupil vectors for file 121 traces [7,35,36]
% load 2017-10-16-001-2-7
% pupil_mat(:,1)=Pupilsize;
% load 2017-10-16-001-4-5
% pupil_mat(:,2)=Pupilsize;
% load 2017-10-16-001-4-6
% pupil_mat(:,3)=Pupilsize;
% 
% pair1=figure;
% ymax=max([plot_data_Vm(:,plot_trace),plot_data_Vm(:,plot_trace)]);
% ymin=min([plot_data_Vm(:,plot_trace),plot_data_Vm(:,plot_trace)]);
...
% ydiffwhisk=ymaxwhisk-yminwhisk;
% yrangewhisk=ceil(max(ydiffwhisk));
% ymaxpupil=max([pupil_mat,pupil_mat]);
% yminpupil=min([pupil_mat,pupil_mat]);
% ydiffpupil=ymaxpupil-yminpupil;
% yrangepupil=ceil(max(ydiffpupil));
% 
% for tr_ind=1:length(plot_trace)
% subplot(3*length(plot_trace),1,3*tr_ind-2)     
%     hold on
% %         p1=plot(interval_plot(:,1).*dt*1000,plot_data_Vm{1}(:,plot_trace(tr_ind)), 'color',color_table(1,:),'LineWidth',1.5);
%        htrace1=plot([segment1*dt, segment2*dt],plot_data_Vm(segment1(1):segment2(end),plot_trace(tr_ind)), 'LineWidth',1.2,'color', color_table(1,:));
%        rec1=rectangle('Position',[segment1(1,1)*dt,plot_data_Vm(segment1(1,1)),0.1,0.1]);
%         rec2=rectangle('Position',[segment1(1,1)*dt,plot_data_Vm(segment1(1,1)),0.1,0.1]);
%    
%         axis tight
...
% horiz_vert=1;        lengthh=1;     textit=[num2str(lengthh), ' s'];     c=[0,0,0];  fonsizes=scalebar_fontsize; perc1=0.03; perc2=[];
%         [b1,b2] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
%  horiz_vert=0;        lengthh=10;     textit=[num2str(lengthh), ' mV'];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=0.03; perc2=[];
%         [b3,b4] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
...
%     set(gca, 'visible', 'off') ;
% 
%     subplot(3*length(plot_trace),1,3*tr_ind-1) 
%  	hold on
%         htrace2=plot([segment1*dt, segment2*dt],plot_data_whisk(segment1(1):segment2(end),plot_trace(tr_ind)), 'LineWidth',1.2,'color', color_table(3,:));
%         
%         axis tight            
%      ylim_data=[get(gca,'ylim')]';
%      ylim_data(1)=ylim_data(2)-yrangewhisk;   
...
%               
%      %plotting scale bar
%              horiz_vert=1;        lengthh=1;     textit=[num2str(lengthh), ' mS'];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=[]; perc2=[];
%                 [b1,b2] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
%              horiz_vert=0;        lengthh=1;     textit=[num2str(lengthh), ' mV'];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=[]; perc2=[];
%                 [b3,b4] = fn_makeCalibBar2(horiz_vert,lengthh,textit,c,fonsizes,perc1,perc2);
%            delete(b1); delete(b2); delete(b3); delete(b4); 
% %          if tr_ind~=length(plot_trace);
% %              delete(b3); delete(b4); 
...
%     set(gca, 'visible', 'off') ;
%     
%   subplot(3*length(plot_trace),1,3*tr_ind) 
%  	hold on
%         htrace3=plot([segment1*dt, segment2*dt],pupil_mat(:,tr_ind), 'LineWidth',1.2);
%         
%         axis tight            
%      ylim_data=[get(gca,'ylim')]';
%      ylim_data(1)=ylim_data(2)-yrangepupil;   
%      xlim_data=[get(gca,'xlim')]';
%      ylim_data(2)=ylim_data(2)+(0.1).*yrangepupil; 
%      set(gca,'ylim',ylim_data, 'xlim',xlim_data)  
%               
%      %plotting scale bar
%              horiz_vert=1;        lengthh=1;     textit=[num2str(lengthh), ' mS'];     c=[0,0,0];  fonsizes=scalebar_fontsize; %perc1=[]; perc2=[];
...
%     title('Vm-whisk single trace','FontSize', 16); 
%     ylabel('Potential (mV)' ,'FontSize', 14); xlabel('Time (s)','FontSize', 14)
% l=legend([htrace1,htrace2,htrace3],{'Vm','Whisk','Pupil'}, 'box', 'off','position',[0.8,0.85,0.2,0.1]); 
%     l.FontSize=11;
% 
% if save_flag==1;    
%             cd(path_output1)
%          filename1= ['f' num2str(files_to_analyze(fileind)) '_VmWhiskPupil_traces_x1'];
%         saveas(pair1,filename1,'fig') 
%         print(pair1,filename1,'-dpng','-r600','-opengl') 
% end
  %% spontaneous figures    
...
htrace1=plot(segment1*dt,trace_to_plot(segment1,:,:), 'LineWidth',1,'color', color_table(1,:));
htrace2=plot(segment2*dt,trace_to_plot(segment2,:,:), 'LineWidth',1,'color', color_table(2,:));
...
htrace1=plot(segment1*dt,plot_data_mean(segment1,:,1), 'LineWidth',1,'color', color_table(1,:));
htrace2=plot(segment2*dt,plot_data_mean(segment2,:,1), 'LineWidth',1,'color', color_table(2,:));
...
htrace1=plot(segment1*dt,plot_data_std(segment1,:,1), 'LineWidth',1,'color', color_table(1,:));
htrace2=plot(segment2*dt,plot_data_std(segment2,:,1), 'LineWidth',1,'color', color_table(2,:));
