...
trace_type_input=[1]; %for exp_type=4 put [1];
...
switch exp_type
    case 1
        files_to_analyze = [8,10,12,14,15,16,22,37,40,1,46,48,52,58,72,82,84]; %[8,10,12,14,15,16,22,36,37,40,1,44,46,48,52,56,58,62,72,75,82,84];  [8,10,12,14,15,16,22,36,37,40,1,44,46,48,52,56,58,62,72,75,82,84];
        QXfiles=[36,37,40,44,46,52,56,58,62,75];
        QX=ismember(files_to_analyze,QXfiles);
    ...
    case 4
        files_to_analyze = [118,121,122,126,127,128,129]; %119
        cd 'D:\Inbal M.Sc\Data PhD\ChAT Data\Extracted Data 2016';
        load ChAT_Files_v3
        legend_string={'Light Off', 'Light On'};  y_ax_label={'Vm'}; y_ax_units={'mV'};    
         path_output=['D:\Inbal M.Sc\Data PhD\ChAT Data\Figures\Traces+std+mean+summary_awake\',path_output];        
        a = exist(path_output,'dir'); %a will be 7 if a folder "name" exists and 0 otherwise
            if a==0;
                mkdir(path_output);
            end
...
% line(tmp_X(:,QX==0),tmp_Y(:,QX==0),'color',[0.7 0.7 0.7],'linewidth',1.5,'markersize',10,'markerfacecolor','k')
% line(tmp_X(:,QX==1),tmp_Y(:,QX==1),'color',[102, 172,255]./256 ,'linewidth',1.5,'markersize',10,'markerfacecolor','k')
