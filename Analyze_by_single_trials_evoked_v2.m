...
switch exp_type
    ...
    case 2
        files_to_analyze =[74,76,77,80,82,84,87,90,92,112,114,115];%[74,76,77,80,82,84,87]; %
          ...
          for trace=1:size(current_data,2)             
              for stim_num=1:11; %1:galvano_nstim;
                  ...
                  if stim_num>size(stim2{t},2); %if there is no test stimulus in this cell
                      ...
                      peak_start_int = round(stim2{t}(1,stim_num)+0.004*sf{1});
                      peak_end_int = round(stim2{t}(1,stim_num)+2*stim_ISI-0.004*sf{1});    
                  end
...
%        end
 ...
 for stim_num=1:size(stim2_X{2},2);
       ...
       ylabel(['Norm Amplitude'],'FontSize', 16);    
        ...
        for stim=stimz
           data_vec=[data_vec,event_evoked_stat.stim_num(stim).change_amplitude_m'];
            ...
            for stim=stimz
                data_mat=[data_mat,event_evoked_stat.stim_num(stim).change_amplitude_m];
            end
            for cells=1:12
                if sum(data_mat(cells,:)>0)==11
                    trend(cells,1)=1;
                else if sum(data_mat(cells,:)<0)==11
                     trend(cells,1)=-1;
                else
                     trend(cells,1)=0;
                    end
            ...
            figure
            plot(1:11,data_mat','linewidth',2,'marker','+')
         Fig1=figure;
            hold on
