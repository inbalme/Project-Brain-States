...
for fileind =[38,51,61,64,67,69,71,74,77]; %,84]; %[17,23,26,27,28,29,33,35,60]; %[31,38,42,51,61,64,67,69,71,74,77]; %[16,22,44,46,48,50,52]; %[32,34,36,37,38,39]
    ...
    if isfield(files,'mode_name')
     ...
     if isfield(files,'mode_val')
        ...
        POG_loc=strcmp('PrimaryOutputGain',Param.mode_name);
          if isnan(Param.mode_val{1,POG_loc==1}) || isempty(Param.mode_val{1,POG_loc==1});
            else
